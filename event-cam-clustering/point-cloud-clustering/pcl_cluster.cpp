...
int main(int argc, char** argv) {
    ...
    // Write to csv
    sprintf(file_name, "event_data_cluster_3090_3110.csv");
    ofstream myfile;
    myfile.open(file_name);

    // reader.read("table_scene_lms400.pcd", *cloud);
    // reader.read("../resources/event_raw_data570-579-offset.pcd", *cloud);
    reader.read("../resources/event_data_3090_3110.pcd", *cloud);
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); it++, j++) {
        ...
        for(std::vector<int>::const_iterator pit = it->indices.begin(); pit != it->indices.end(); ++pit) {
            ...
            myfile << tmp.x << "," << tmp.y << "," << tmp.z << "," << tmp.intensity << endl;
        }
    }
    ...
    writer.write<pcl::PointXYZI>("cloud_clustered_event.pcd", *cloud_clustered, false);
