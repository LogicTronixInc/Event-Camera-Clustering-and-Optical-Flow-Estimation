...
struct TrackedCorner {
    ...
    int frame_count;
    bool is_matched;
    std::deque<cv::Point> position_history;  // Store previous positions
};
...
public:
    ...
    std::vector<TrackedCorner> updateTrackedCorners(const std::vector<Corner>& current_corners) {
        ...
        for (const auto& corner : current_corners) {
            ...
            current_tracked_corners.push_back(tracked);
        }
        ...
        for (auto& current : current_tracked_corners) {
            ...
            if (best_match != nullptr) {
                ...
                current.position_history = best_match->position_history;
                updatePositionHistory(current);
...
public:
    static cv::Mat visualizeTrackedCorners(
        ...
        for (const auto& corner : corners) {
            ...
            if (corner.frame_count >= 3) {  // Adjust this threshold as needed
                ...
                if (corner.position_history.size() > 1) {
                    ...
                    if (corner.position_history.size() >= 2) {
                        cv::Point start = corner.position_history[1];  // Previous position
                        cv::Point end = corner.position_history[0];    // Current position
                        
                        // Calculate arrow tips
                        double angle = atan2(end.y - start.y, end.x - start.x);
                        double arrow_length = 10.0;
                        double arrow_angle = CV_PI / 6;  // 30 degrees
                        
                        cv::Point arrow1(
                            end.x - arrow_length * cos(angle + arrow_angle),
                            end.y - arrow_length * sin(angle + arrow_angle)
                        );
                        cv::Point arrow2(
                            end.x - arrow_length * cos(angle - arrow_angle),
                            end.y - arrow_length * sin(angle - arrow_angle)
                        ...
                        // Draw arrow head
                        cv::line(output, end, arrow1, color, 1, cv::LINE_AA);
                        cv::line(output, end, arrow2, color, 1, cv::LINE_AA);
                    }
                }
            }
        }
...
int main(int argc, char *argv[])
{
    ...
    // Initialize tracker
    CornerTracker tracker(30.0f, 30, 10);  // 30.0f is max matching distance
