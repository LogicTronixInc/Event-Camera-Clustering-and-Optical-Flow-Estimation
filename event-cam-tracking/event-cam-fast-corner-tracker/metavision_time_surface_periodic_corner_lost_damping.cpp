...
{
    cv::Point2f current; // Current direction
    cv::Point2f target;  // Target direction
    ...
    {
        target = new_target;
        // Smooth transition to new target direction
...
private:
    ...
    float damping_factor;   // Control direction vector damping
    float smoothing_factor; // Control direction smoothing
        ...
        const std::vector<TrackedCorner> &corners)
    {
        ...
        for (const auto &corner : corners)
        {
            if (corner.frame_count >= 3)
            {
                ...
                float direction_scale = 50.0f; // Scale factor for direction vector visualization
                cv::Point2f dir_end(
                    corner.x + corner.direction.current.x * direction_scale,
                    corner.y + corner.direction.current.y * direction_scale);
                cv::arrowedLine(output,
                                cv::Point(corner.x, corner.y),
                                cv::Point(dir_end.x, dir_end.y),
                                fade_color, 2, cv::LINE_AA);
                ...
                if (corner.position_history.size() > 1)
                {
                    for (size_t i = 1; i < corner.position_history.size(); ++i)
                    {
                        float path_alpha = alpha * (1.0f - static_cast<float>(i) / corner.position_history.size());
                        cv::Scalar path_color = color * path_alpha;
                        cv::line(output,
                                 corner.position_history[i - 1],
                                 corner.position_history[i],
                                 path_color, 1, cv::LINE_AA);
                    }
                }
