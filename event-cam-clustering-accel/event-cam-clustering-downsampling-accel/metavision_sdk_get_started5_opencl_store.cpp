    ...
    reslicer.set_on_new_slice_callback([&](Metavision::EventBufferReslicerAlgorithm::ConditionStatus status,
                                           Metavision::timestamp ts, std::size_t nevents)
                                       {
                                        ...
                                        //   printf("data at index 16383 : %d\n", data[16383]);
                                          ...
                                          uniqueCount_diff = uniqueCount - uniqueCount_prev;
                                        //   printf("uniqueCount_diff : %d\n", uniqueCount_diff);
                                        //     // std::cout << "\nSample Repeated Coordinates:\n";

                                            // std::cout << "\nSample Unique Coordinates:\n";
                                            for (int i = 0; i < uniqueCount_diff; i+=4)
                                            {
                                                
                                                // std::cout  << uniqueCoords[i] << ","<< uniqueCoords[i+1] << std::endl;
                                                ev_data[0] = uniqueCount/1000.0;       
                                                ev_data[1] = uniqueCoords[i];
                                                ev_data[2] = uniqueCoords[i+1];
                                                ev_data[3] = 0;
                                        ...
                                        /* Deallocate event */
                                            ...
                                            for (auto cc : eclustering->clusters){
                                                ...
                                                if (cc.getN() >= minN){
                                                    ...
                                                    std::cout << "after initial get(" << centroid_prev[cc.getClusterId()][0] << " , " << centroid_prev[cc.getClusterId()][1] << ")" << "," ; 
                                                    
                                                    cv::circle(cluster_img,cv::Point(centroid_prev[cc.getClusterId()][0],centroid_prev[cc.getClusterId()][1]), 1, cv::Scalar(0,0,255,255), -1);
                                                    ...
                                                    double centroid_diff_x = cen[0] - centroid_prev[cc.getClusterId()][0];
                                                    double centroid_diff_y = cen[1] - centroid_prev[cc.getClusterId()][1];

                                                    // std::cout << centroid_diff_x << "," << centroid_diff_y << std::endl;
                                                    if (centroid_prev[cc.getClusterId()][0]>0 && centroid_prev[cc.getClusterId()][1]>0){
                                                        cv::arrowedLine(cluster_img, cv::Point(centroid_prev[cc.getClusterId()][0], centroid_prev[cc.getClusterId()][1] ), 
                                                                    cv::Point((centroid_prev[cc.getClusterId()][0]+centroid_diff_x), (centroid_prev[cc.getClusterId()][1]+centroid_diff_y)),
                                                    ...
                                                    centroid_prev[cc.getClusterId()][0] = cen[0];
                                                    centroid_prev[cc.getClusterId()][1] = cen[1];
                                            ...
                                            // for (int i = 0; i < uniqueCount_diff; i+=2)
                                            // {                                             
                                           
                                            //     cv::circle(cluster_img,cv::Point(uniqueCoords[i],uniqueCoords[i+1]), 1, cv::Scalar(0,0,255,255), -1);
                                            // }
